...
double solution::objectiveFunctionW (double  & lambda, double & mu){
	...
	cilk_for (int i = 0; i < _Q; i++){   /*NOTHIN CILK CANDO HERE*/
		for (int j = 0; j < _N; j++){
			if (_observation[i][j] != -1){
				...
				sumLog += ((_observation[i][j])? log(1/(1+exp(-WiCj))): log(1-1/(1+exp(-WiCj))));
			}
		}
		double sumTemp(0.0);
		for (int k = 0; k < _K; k++){
...
double solution::objectiveFunctionC (double & gamma){
	...
	cilk_for (int i = 0; i < _K*_N; i++){
			sumC += _C[i*_N+i%_N]*_C[(i%_K)*_N+i%_N];
	}
...
{
    double expsum = 1.0; // initialize sum of series
int  n=5; // your chice of to what level yopyu want top approximate 
    for (int i = n - 1; i > 0; --i )
        expsum = 1 + x * expsum / i;
 
    return expsum;
}
long  sumLogCalc(int i,long &sumLog){
{
float WiCj;
int obs_ij;             //observation data
double logesticFunc;    // = 1/1+exp(...)
...
int iter=i%_Q;
iter_1 = (iter) * _K;
           obs_ij = _observation[iter][jter];
           if (obs_ij != -1){
                   WiCj = 0.0;
                   for (k=0; k < _K; k++){
                           WiCj += _W[iter_1+k]*_C[i%_N+k*_N]; //spawn()
                   }
